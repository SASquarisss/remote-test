...
import { ENTITY_DATA, ZH_LABELS, ENTITY_STYLES } from '../data/schema.js';
...
export class OntologyLegend {
  ...
  render() {
    ...
    let html = '';
    const rootNodes = Object.keys(ENTITY_DATA).filter(k => !ENTITY_DATA[k].is_a);
    ...
    rootNodes.forEach(root => {
      ...
      html += `<span style="width: 12px; height: 12px; border-radius: 50%; background: ${ENTITY_STYLES[root]?.color || '#ccc'}; border: 1px solid #aaa;"></span>`;
      html += `${escapeHtml(rootZh)} <span style="color:#888;font-size:12px;font-weight:normal;">(${escapeHtml(root)})</span></div>`;
      
      const children = Object.keys(ENTITY_DATA).filter(k => ENTITY_DATA[k].is_a === root);
      if (children.length > 0) {
        ...
        children.forEach(child => {
          ...
          html += `<span style="width: 10px; height: 10px; border-radius: 4px; background: ${ENTITY_STYLES[child]?.color || '#ccc'};"></span>`;
